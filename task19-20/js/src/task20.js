...
console.log('clear json data:', data);

//1.skills massive

var skills = _.map(data, 'skills'); //find skills
var skillsMassive=[]; //total skill massive
var eachSkill=[]; //massive with each skill

_.forEach(skills, function(key,value){ 
	skillsMassive=_.concat(skillsMassive, skills[value]);
});

eachSkill = _.uniq(skillsMassive); //find unique skills
 
var alphabetSort = _.sortBy(eachSkill, function (value) { 
	return _.toLower(value); 
});
console.log('skills sorted by alphabet:', alphabetSort);


//2.names massive

var arrNames = _.map(data, 'name'); //find name
var sortFriends = _.sortBy(data, ['friends' , 'name']);
var sortNames = _.map(sortFriends, 'name'); // sort names by frieds quantity
console.log('Sorted names by friends quantity: ', sortNames);


//3.friends massive

var friends = _.map(data, 'friends'); //find friends
var friendsListObj=[];
var uniqueFriends=[];


_.forEach(friends, function(key,value){  //get friend object
	friendsListObj=_.concat(friendsListObj, friends[value]);
});

var friendsList = _.map(friendsListObj, 'name'); //get friendlist from object

uniqueFriends = _.uniq(friendsList);
console.log('List of friends without repeat:', uniqueFriends);
